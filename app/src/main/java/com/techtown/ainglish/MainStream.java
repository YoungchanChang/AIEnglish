...
import com.techtown.ainglish.Adapter.OnTeacherUserItemClickListener;
import com.techtown.ainglish.Adapter.TeacherUserAdapter;
...
public class MainStream extends AppCompatActivity {
    ...
    private static final String TAG = "MainStreamingLog";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.main_stream);
    ...
    //채팅목록창을 위한 리싸이클러뷰이다.
    RecyclerView recycler_my_teacher;
    TeacherUserAdapter teacher_all_adapter;
    ...
    public void setRecycler(){
        ...
        // 1.레이아웃 설정
        recycler_my_teacher = findViewById(R.id.recycler_streaming);
        recycler_my_teacher.setLayoutManager(layoutManager);
        // 2.아답터 결합
        teacher_all_adapter = new TeacherUserAdapter(this);
        recycler_my_teacher.setAdapter(teacher_all_adapter);

        //클릭시 실제 스트리밍하는 곳으로 보낸다.
        teacher_all_adapter.setOnItemClickListener(new OnTeacherUserItemClickListener() {
            @Override
            public void onItemClick(TeacherUserAdapter.ViewHolderTeacherUser holder, View view, int position) {

                Log.d(TAG, "onItemClick: Teacher클릭됨");
                //여기 변경
                TeacherInfoJSON item = teacher_all_adapter.getItem(position);

                Intent go_rtmp = new Intent(MainStream.this, StreamRoom.class);
                go_rtmp.putExtra("teacher_info", item.getTeacher_id());
                go_rtmp.putExtra("user_info", shared_user_info.getServer_id());
                go_rtmp.putExtra("position", "student");
                go_rtmp.putExtra("user_profile", shared_user_info.getServer_image());
                go_rtmp.putExtra("user_nickname", shared_user_info.getServer_nickname());

                startActivity(go_rtmp);
        ...
        SingletonNewHttp.getInstance().putURI(SingletonNewHttp.TEACHER_USER_U_STREAM_FIND);
        //성공시 처리한다.
        SingletonNewHttp.getInstance().setHttpProperty(MainStream.this, response_listener_teacher);
    ...
    public void initInfoSet(){
        ...
        //성공시 처리한다.
        SingletonNewHttp.getInstance().setHttpProperty(MainStream.this, response_listener);
    ...
    Response.Listener<String> response_listener_user;
    Response.Listener<String> response_listener_teacher;

    public void setListener(){
        response_listener_teacher =  new Response.Listener<String>(){
            @Override
        ...
        response_listener_user =  new Response.Listener<String>(){
            @Override
            public void onResponse(String response) {
